...
class ImageTestCase(BaseTestCase):
    ...
    def test_create_image_when_wrong_market_id(self, validator):
    ...
    def test_create_image_when_when_not_valid(self):
        ...
        error = self.client.post('/api/images/',
                                 follow_redirects=True,
                                 data=data)
        assert error.json()["error"] == dict(market_id= ['"123" is not a valid UUID.'],
                                         image_path=['No file was submitted.'])
